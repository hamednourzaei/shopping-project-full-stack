...
import { Button, Flex, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, Stack, Text, useDisclosure } from '@chakra-ui/react'; // اضافه کردن useDisclosure
import React, { useEffect, useState } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { ADD_Cloth_item, Get_cloth_item, REMOVE_Cloth_item, UPDATE_Cloth_item } from '../../store/Cloth/Cloth.action';

const Clothes = () => {
  const { isOpen, onOpen, onClose } = useDisclosure(); // استفاده از useDisclosure
  ...
  // فیلتر کردن اقلام بر اساس دسته‌بندی
  useEffect(() => {
    const filteredMen = cloth.filter((item) => item.category === filter);
    console.log(filteredMen); // برای بررسی فیلتر
  }, [filter, cloth]);
